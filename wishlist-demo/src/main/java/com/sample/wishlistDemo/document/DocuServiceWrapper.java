...
public class DocuServiceWrapper {
    ...
    public Wishlist[] getWishlists() {
        try {
            ...
            Wishlist[] wishlists = new RestTemplate().exchange(url, HttpMethod.GET, request, Wishlist[].class).getBody();
            return wishlists;
    ...
    public String postWishList(Wishlist wishlist) {
        try {
            String token = getTokenForServiceToCallDocuRepoOnBehalfOfProject();
            String url = getBaseUrl();
            HttpEntity<String> request = new HttpEntity<>(toJson(wishlist), headerWithToken(token));
    ...
    public Optional<Wishlist> getWishlistById(String id) {
        try {
            ...
            Wishlist wishlist 
             = new RestTemplate().exchange(url, HttpMethod.GET, request, Wishlist.class).getBody();
